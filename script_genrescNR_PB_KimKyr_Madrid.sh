...
#script_genrescNR_PB_KimKyr.sh <d> <LAMB> <BETA> <AVEs> <AVEh>
#To be used only in a machine with /state/partition1 directory

rm script_genrescNR_PB_KimKyr.sh.*
...
if [ $# -ne 5 ]  
then
	echo "Usage: $0 <d> <LAMB> <BETA> <AVEs> <AVEh>" 
	exit 1
fi
...
#Working directory
WDIR=$PWD 
mkdir -p $WDIR/genrescPB_results/KymKyr/L$LAMB.k$NCRO.s$AVEs.h$AVEh.beta$BETA/TP$NINDTP.AL$NINDAL.M$MIG.INT$INT.g$tR
DIR="genrescPB_results/KymKyr/L$LAMB.k$NCRO.s$AVEs.h$AVEh.beta$BETA/TP$NINDTP.AL$NINDAL.M$MIG.INT$INT.g$tR"
...
cp genrescNR_PB_KimKyr $SCDIR/$SLURM_JOBID/
...
START=$(date +%s)
time ./genrescNR_PB_KimKyr>>out<<@
0
-99
10000	NIND Natural Population
...
$BETA	beta_s
$AVEs	ave |s|
2	dom model (0=cnt; 1:variable, 2:Kim_Kyriacis)
$AVEh	ave h
